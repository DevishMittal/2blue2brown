...
async def run_clip_agent(index, scene, grant_instance):
    ...
    prompt = (
        ...
        "- CRITICAL: Each scene must last AT LEAST 15-20 seconds. Add sufficient wait times (self.wait(3) or longer) after each animation.\n" 
        "- Add at least 5 seconds (self.wait(5)) at the end of each scene to ensure proper viewing time.\n"
        "- Ensure all text and visual elements are properly spaced and don't overlap by carefully positioning elements.\n"
        "- Set appropriate font sizes (30-40 for main text, 24-30 for labels) to ensure readability.\n"
